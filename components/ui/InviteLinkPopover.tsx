  ...
  useEffect(() => {
    ...
    function handleClick(e: MouseEvent) {
      if (btnRef.current && !btnRef.current.contains(e.target as Node)) {
        const popover = document.getElementById("creds-popover");
      ...
      {open && (
        <motion.div
          ...
          style={{
            position: "absolute",
            top: pos.top,
            right: `calc(100vw - ${pos.left}px + 8px)`,
          }}
          className="w-72 z-[9999] rounded-xl bg-[#0e0e16] border border-white/[0.1] shadow-2xl shadow-black/60 overflow-hidden"
        >
          ...
          <div className="p-3 space-y-2.5">
            ...
            {error && (
              <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 px-2.5 py-2 rounded-lg">{error}</p>
            )}
            {creds && (
              <>
